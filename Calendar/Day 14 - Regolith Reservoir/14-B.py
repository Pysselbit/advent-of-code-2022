...
def load_grid(path):
    ...
    # Add pseudo-infinite floor (and update min and max):
    floor_margin = 10000
    floor_drop = 2
    floor_a = Pos(x_min - floor_margin, y_max + floor_drop)
    floor_b = Pos(x_max + floor_margin, y_max + floor_drop)
    structures.append([floor_a, floor_b])
    x_min -= floor_margin
    x_max += floor_margin
    y_max += floor_drop
...
sand_source_x = SAND_SOURCE_X + input[DX]
sand_source_y = SAND_SOURCE_Y + input[DY]

# Add sand until sand source is blocked by sand:
sand_unit_count = 0
while input[GRID][sand_source_y][sand_source_x] != SAND:
    add_sand_unit(Pos(sand_source_x, sand_source_y), input[GRID])
    sand_unit_count += 1
