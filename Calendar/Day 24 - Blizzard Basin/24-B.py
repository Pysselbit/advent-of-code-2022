...
start_y, final_goal_y = len(map) - 1, 0
start_x, final_goal_x = map[start_y].index(0), map[final_goal_y].index(0)
goal_x, goal_y = final_goal_x, final_goal_y
...
has_reached_goal = [False, False, False]
while False in has_reached_goal and len(queue) > 0:
    visit = queue.pop(0)

    # See if goal has been reached (and update goal if needed):
    if visit[X] == goal_x and visit[Y] == goal_y:
        if not has_reached_goal[0]:
            has_reached_goal[0] = True
            goal_x, goal_y = start_x, start_y
            queue = [(final_goal_x, final_goal_y, total_time + 1)]
        elif not has_reached_goal[1]:
            has_reached_goal[1] = True
            goal_x, goal_y = final_goal_x, final_goal_y
            queue = [(start_x, start_y, total_time + 1)]
        else:
            has_reached_goal[2] = True
            break
